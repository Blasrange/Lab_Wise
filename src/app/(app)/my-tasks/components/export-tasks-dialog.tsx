...
import type { EquipmentHistory } from "@/lib/types";
import { useToast } from "@/hooks/use-toast";
import { exportTasksToExcel } from "@/lib/exportUtils";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function ExportTasksDialog({
  ...
  const [exportOption, setExportOption] = useState<"pending" | "completed">(
    "pending"
  );
  ...
  const handleDownload = () => {
    ...
    try {
      const tasksToFilter =
        exportOption === "pending" ? pendingTasks : completedTasks;
      const sheetTitle =
        exportOption === "pending"
          ? t("my_tasks_page.pending_title")
          : t("my_tasks_page.completed_title");

      const filteredTasks = tasksToFilter.filter((item) => {
      ...
      exportTasksToExcel(filteredTasks, sheetTitle, t);
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <DialogTitle>{t("my_tasks_page.export.title")}</DialogTitle>
              <DialogDescription>
                {t("my_tasks_page.export.description")}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 py-4">
            ...
            <Card className="h-full">
              ...
              <CardContent className="space-y-4">
                <Alert>
                  ...
                  <AlertDescription>
                    {t("my_tasks_page.export.info_alert")}
                  </AlertDescription>
                </Alert>
                <Alert
                  variant="default"
                ...
                >
                  ...
                  <AlertDescription className="text-blue-700 dark:text-blue-400">
                    {t("my_tasks_page.export.report_content_desc")}
                  </AlertDescription>
                </Alert>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  {t("my_tasks_page.export.select_option_title")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <RadioGroup
                  value={exportOption}
                  onValueChange={(value) =>
                    setExportOption(value as "pending" | "completed")
                  }
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="pending" id="r1" />
                    <Label htmlFor="r1">
                      {t("my_tasks_page.pending_title")} ({pendingTasks.length})
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
